function y = OKmse_kd_var(X_0, X, sigma_z, Theta, Cov_Inverse)
...
for c = 1:size(X_0,1)
    ...
    temp(c) = max(sigma_z.*( 1- Cov_Pred'*Cov_Inverse*Cov_Pred + ((1-F'*Cov_Inverse*Cov_Pred).^2)/(F'*Cov_Inverse*F)),0);
    if temp(c)==0
        temp(c) = abs(normrnd(0,0.0001));
    end
